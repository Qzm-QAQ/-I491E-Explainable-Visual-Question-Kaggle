...
class QwenVLDataCollator:
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
        ...
        # 对每个样本，把 user 那段 label 设成 -100，只训 assistant 输出
...
def parse_answer_explanation(text: str):
    ...
    def normalize_answer(ans: str) -> str:
        ...
        - 去掉多余标点和空格
        - 只保留首个 token
        - 英文数字 -> 数字字符，grey->gray 等
        """
        ans = ans.strip().lower()
        # 只保留第一行
        ans = ans.splitlines()[0].strip()
        # 删掉后面误带上的 explanation:
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to custom_dataset directory (contains train/test/*.csv).",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
